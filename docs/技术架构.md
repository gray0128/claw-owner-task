...
### 6. Telegram 机器人集成 (Conversational AI & Push)
- **双向通信**: 
  - **下行 (Push)**: 集成在提醒引擎中，支持 Markdown (HTML Mode) 格式排版。
  - **上行 (Interaction)**: 通过 Webhook 端点接收消息，复用后端的 AI 处理逻辑。
- **安全性**: 
  - **白名单机制**: 仅受理预设的 `TELEGRAM_CHAT_ID` 的消息。
  - **鉴权隔离**: Webhook 路径在主鉴权中间件中设为白名单，由业务逻辑内部校验来源安全性。
- **UI 适配**: AI 返回结果会自动转义并按聊天窗口的使用习惯进行结构化排版（加粗、列表）。
