...
graph TB
    ...
    Chatbots -->|发送文本与语音指令| Webhook
    Webhook -->|语音下载并保存为临时对象| R2
    Webhook -->|构造内部安全代理URL调用| Volcengine
    Volcengine -->|通过代理回源拉取音频| Workers
    Workers -->|从存储拉取文件流| R2
    Workers -->|1. 返回转录文本注入 AI| Webhook
    Workers -->|2. Finally 阅后即焚| R2
...
sequenceDiagram
    ...
    participant Bot as 飞书 / Telegram
    participant Worker as Cloudflare Worker
    participant R2 as R2 Bucket
    participant Volc as 火山引擎 ASR
    participant AI as AI Handler (Cloudflare AI)

    User->>Bot: 发送语音消息
    Bot->>Worker: Webhook 事件推送
    Worker-->>Bot: HTTP 200 OK (立即响应)
    
    rect rgb(240, 240, 240)
        Note over Worker: waitUntil (后台异步处理)
        Worker->>Bot: 下载音频资源 (ArrayBuffer)
        Bot-->>Worker: 音频二进制数据
        
        Worker->>R2: 上传临时文件 (UUID Key)
        R2-->>Worker: 确认上传成功
        
        Worker->>Volc: 提交 ASR 任务 (携带 Worker 代理 URL)
        Volc-->>Worker: 任务提交成功 (RequestID)
        
        Volc->>Worker: GET /api/proxy/audio/... (回源拉取)
        Worker->>R2: 读取音频对象
        R2-->>Worker: 文件流
        Worker-->>Volc: 流式返回音频数据
        
        loop 轮询识别结果
            Worker->>Volc: 查询任务状态
            Volc-->>Worker: 识别中 / 识别成功 (Text)
        end
        
        Worker->>AI: 注入转录文本进行语义解析
        AI-->>Worker: 解析动作 (Task Action)
        
        Worker->>R2: DELETE 物理删除临时文件 (阅后即焚)
        Worker->>Bot: 发送最终处理结果通知
    end
    Bot-->>User: 显示任务处理反馈
```
...
```mermaid
sequenceDiagram
    participant CloudCron as Cloudflare Cron (Trigger)
    participant Worker as Worker (scheduled)
    participant DB as D1 Database
    participant AI as Cloudflare AI (AI_MODEL)
    participant Notify as Notification Service
    participant Channels as Channels (Bark/TG/Feishu)

    CloudCron->>Worker: 触发定时任务 (每分钟)
    Worker->>Worker: 检查当前时间是否匹配 CRON_SUMMARY_TIME
    
    alt 时间匹配 (例如 09:00 或 21:00)
        Worker->>DB: 查询所有待处理 (pending) 及进行中 (in_progress) 任务
        DB-->>Worker: 返回任务列表 JSON
        
        Worker->>AI: 提交任务列表，请求高维度总结 (JSON 格式)
        AI-->>Worker: 返回结构化总结数据
        
        Note over Worker: 生成唯一 UUID
        Worker->>DB: 存储总结 JSON，设置 24 小时过期
        DB-->>Worker: 确认存储
        
        Worker->>Notify: 调用 sendToAllChannels (source=cron)
        
        par 广播推送
            Notify->>Channels: 推送 Bark (iOS)
            Notify->>Channels: 推送 Telegram (HTML)
            Notify->>Channels: 推送 飞书 (Text)
        end
        
        Channels-->>Worker: 推送完成
    else 时间不匹配
        Worker-->>CloudCron: 结束任务
    end
```
...
sequenceDiagram
    participant User as 用户
    participant Webhook as Bot Webhook (Feishu/TG)
    participant Parser as parseCommand (bot-shared)
    participant Internal as 业务 Handler (Tasks/Summary)
    participant AI as Cloudflare AI
    participant BotAPI as Bot Platform API

    User->>Webhook: 发送 "/add 买牛奶" 或 "我想买牛奶"
    Webhook-->>User: HTTP 200 OK (立即响应，防止重试)

    rect rgb(240, 240, 240)
        Note over Webhook: waitUntil (后台异步处理)
        Webhook->>Parser: 解析文本意图
        
        alt 匹配快捷指令 (Slash Command)
            Parser-->>Webhook: 返回命令名 (add) 及参数 (买牛奶)
            Webhook->>Internal: 直接分发至对应业务逻辑 (Bypass AI)
            Internal-->>Webhook: 返回执行结果 (Success/Fail)
        else 无指令前缀 (Natural Language)
            Parser-->>Webhook: 返回非指令状态
            Webhook->>AI: 调用 AI Handler 进行语义解析
            AI->>Internal: AI 提取意图后流转至业务逻辑
            Internal-->>AI: 返回执行结果
            AI-->>Webhook: 返回格式化后的 AI 响应
        end
        
        Webhook->>BotAPI: 调用发送消息接口反馈给用户
    end
    BotAPI-->>User: 显示最终执行结果
```
...
- **语音消息处理与 ASR 集成 (v2 架构)**: 
  - 统一支持飞书与 Telegram 语音消息的接收。
  - **数据流转**: 
    1. `Bot Webhook 提取语音资源并异步下载为 ArrayBuffer`。
    2. `利用不可预测的 UUID 作为键名上传至 Cloudflare R2 私有存储桶，保留 Content-Type`。
    3. `拼接构造指向 Worker 内部 /api/proxy/audio/... 的代理 URL`。
    4. `提交代理 URL 调用火山引擎 ASR 轮询转录结果，将结果注入 AI 处理管线`。
    5. `Finally 块确保触发阅后即焚，立刻物理删除 R2 中的临时文件对象`。
  - **防盗链代理层**: `audio-proxy.ts` 提供了一个受限公开的读取通道供 ASR 引擎下载。由于代理 URL 中包含强随机哈希，即使它是开放的且不带传统签名参数，也具备了“通过隐蔽性保障的安全 (Security by Obscurity)”，使得临时音频安全地对 ASR 透传。
...
- **防止 Webhook 超时重试 (Timeout Prevention)**:
  - 处理 AI 总结、ASR 语音转录或复杂语义解析时通常耗时较长（可能超过聊天平台如飞书的 3 秒 Webhook 超时限制）。
  - 利用 Cloudflare Workers 的 `executionCtx.waitUntil(...)` 将 AI 推理、语音下载、云端交互和消息发送全面转入后台异步执行，Webhook 主线程立即返回成功状态（如 `200 OK`），从根本上切断了聊天平台因为超时而不断重试发送消息导致的大量重复推送风暴。
...
- **Bot 网关** (`telegram.ts`, `feishu.ts`)，**及通用逻辑** (`bot-shared.ts`)：实现不同平台的原生 Webhook。这 2 个平台通信实现文件深度复用 `bot-shared.ts` 抽象出的核心能力集（统一 `/add` 和 `/summary` 指令识别提取，边缘 Isolate 挂起心跳过期监测，内部对象的规范化提取等）。它们同时还负责在后台异步下载语音并与 `AUDIO_BUCKET` (R2) 交互。
- **音频流代理** (`audio-proxy.ts`)：彻底重构为纯粹的 **R2 对象存储代理网关**。不再负责下载外部资源，而是专门利用 `c.env.AUDIO_BUCKET.get` 读取已经下载并上传到 R2 的内部对象，提供给外部 ASR 服务带有 Content-Type 的流式直读。
...
- 2026-04-14 12:00:00: 将 hardcode 的 Cloudflare AI 模型抽取为环境变量 AI_MODEL 配置，并在系统架构图中体现可配置化。
