...
- **长度约束与截断逻辑**：
    1. **设定安全阈值**：基于默认模型 `glm-4.7-flash` 的 131,072 Tokens 限制，为任务上下文预留约 **80,000 Tokens** 的安全空间（折合中英混合文本约 **120,000 字符**），以确保留有充足空间处理系统提示词、用户输入及模型输出。
...
## 变更记录
- 2026-04-14 17:58:10：将 hardcode 的 Cloudflare AI 模型（glm-4.7-flash）抽取为环境变量 `AI_MODEL` 配置，本说明中的模型提及均调整为“默认模型”。
