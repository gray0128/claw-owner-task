...
4. **利用分享链接**：在创建、更新或查询单条任务后，响应中包含 `view_url`。AI 可以将此链接提供给人类用户，以便用户在美观的网页界面查看详情。
5. **标签无需预创建**：直接在 `--tags` 传入名称，系统会自动创建不存在的标签。
...
```json
{
  ...
  "view_url": "https://claw-task.pages.dev/share/uuid-here"
}
```
